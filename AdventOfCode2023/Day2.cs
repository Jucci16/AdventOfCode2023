...
public class Day2
{
    private static Dictionary<string, int> _colorDict = new Dictionary<string, int>
    {
        {"red", 12 },
        {"green", 13 },
        {"blue", 14 }
    ...
    private static int ValidMatchesIndex(string line)
    {
        var matches = line.Split(":")[1].Split(";");
        foreach (var match in matches)
        {
            if (!IsValidMatch(match)) return 0;
        }

        return GetGameIndex(line);
    }

    private static bool IsValidMatch(string match)
    {
        match = match.Trim();
        var colorCountCollection = match.Split(",");
        foreach (var colorCount in colorCountCollection)
        {
            if (!IsValidColorCount(colorCount.Trim())) return false;
        }

        return true;
    }

    private static bool IsValidColorCount(string colorCount)
    {
        var split = colorCount.Split(" ");
        var color = split[1];
        var num = int.Parse(split[0]);
        return num <= _colorDict[color];
    }

    private static int FindPower(string line)
    {
        var matches = line.Split(":")[1].Split(";");
        var maxRed = 0;
        var maxGreen = 0;
        ...
        foreach (var match in matches)
        {
            maxRed = Math.Max(maxRed, GetMatchColor(match, "red"));
            maxGreen = Math.Max(maxGreen, GetMatchColor(match, "green"));
            maxBlue = Math.Max(maxBlue, GetMatchColor(match, "blue"));
        }

        return maxRed * maxGreen * maxBlue;
    }

    private static int GetMatchColor(string match, string colorMatch)
    {
        var groupings = match.Split(",");
    ...
    public static string Input() => @"Game 1: 4 red, 3 blue; 6 blue, 16 green; 9 blue, 13 green, 1 red; 10 green, 4 red, 6 blue
